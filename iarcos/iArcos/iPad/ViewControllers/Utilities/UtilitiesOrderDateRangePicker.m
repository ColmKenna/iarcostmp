//
//  UtilitiesOrderDateRangePicker.m
//  Arcos
//
...
@implementation UtilitiesOrderDateRangePicker
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    self.startDatePicker.date = self.orderStartDate;
    self.endDatePicker.date = self.orderEndDate;
...
-(IBAction)datePicked:(id)sender{
    ...
    if (picker.tag==0) {
        self.orderStartDate = picker.date;
        startDateLabel.text=dateString;
    }
    if (picker.tag==1) {
        self.orderEndDate = picker.date;
...
-(IBAction)save:(id)sender {
    [self.delegate utilitiesDateSelectedForm:self.startDatePicker.date To:self.endDatePicker.date ];
}
