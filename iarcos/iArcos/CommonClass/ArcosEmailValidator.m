...
+(BOOL)checkCanSendMailStatus:(UIViewController*)aTarget {
    if (![MFMailComposeViewController canSendMail]) {
        [ArcosUtils showDialogBox:@"Please set up a Mail account in order to send email" title:@"No Mail Account" delegate:nil target:aTarget tag:0 handler:^(UIAlertAction *action) {}];
        return NO;
    }
    return YES;
}
